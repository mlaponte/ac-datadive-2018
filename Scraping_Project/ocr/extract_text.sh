#!/bin/bash
#Sauce: https://diging.atlassian.net/wiki/spaces/DCH/pages/5275668/Tutorial+Text+Extraction+and+OCR+with+Tesseract+and+ImageMagick
    ...
    if [[ $FILESIZE -lt $MIN_WORDS ]]
        then
            ...
            convert -density 300 "$FILEPATH" -depth 8 -strip -background white \
                    -alpha off /src/temp.tiff > /dev/null 2>&1
            # Then use Tesseract to perform OCR on the tiff.
            tesseract /src/temp.tiff "$OUTFILE" -l $LANG > /dev/null 2>&1
